...
var moveFiles = function(callback) {
  ...
  function(callback) {
    ...
    // 移动最终合并后的目标css文件
    ncp(path + '/css/global.css', staticFolder + '/' + version + '/css/global.css', callback);
  }, function(callback) {
    
    // 移动当页的css文件
    ncp(path + '/css/page', staticFolder + '/' + version + '/css/page', callback);
  }, function(callback) {
    ...
    // 移动图片文件
    ncp(path + '/img', staticFolder + '/' + version + '/img', callback);
  }], callback);
};
